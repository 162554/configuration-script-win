...
function Install-OtherSoftWare {
    ...
    $sofearename = $softwareurl.split("/")[-1]
    Write-Host "sofearename is  $($sofearename )" -ForegroundColor Green
    $outpath = "$env:USERPROFILE\Downloads\$sofearename"
    ...
    foreach($obj in $InstalledSoftware){
        if($sofearename.Contains($obj.GetValue('DisplayName'))){
            Write-Host "$sofearename is already installed!" -ForegroundColor Green
            break
        }
        else {
            Write-Host "Attempting to install: $sofearename..." -ForegroundColor Green
...
Write-Host "请先确保已登录微软账号和onedrive账号，" -ForegroundColor Yellow
...
if ($pressedKey -eq 'c') {
    ...
    ## Reimage 后应该先更新商店再安装winget，否则找不到winget包
    # Install Winget
